...
impl WalletKey {
    /// Create a new wallet key (encrypts the private key).
    pub fn new(address: String, private_key: &[u8], encryption_key: &[u8]) -> Result<Self> {
        ...
        let encrypted = Self::encrypt(private_key, encryption_key, &salt)?;
    ...
    fn encrypt(data: &[u8], key: &[u8], salt: &[u8]) -> Result<Vec<u8>> {
    ...
    fn decrypt_data(encrypted: &[u8], key: &[u8], salt: &[u8]) -> Result<Vec<u8>> {
        // XOR is symmetric
        Self::encrypt(encrypted, key, salt)
    }
...
mod tests {
    ...
    fn test_wallet_key_encryption() {
        ...
        let wallet_key =
            WalletKey::new("0x1234".to_string(), private_key, master_key).unwrap();
        ...
        // Wrong key should give wrong result
        let wrong_decrypted = wallet_key.decrypt(b"wrong-key").unwrap();
        assert_ne!(wrong_decrypted, private_key.to_vec());
    }
}
