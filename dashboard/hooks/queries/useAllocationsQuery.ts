...
export function useAllocationsQuery(workspaceId: string | undefined) {
  return useQuery({
    queryKey: workspaceId ? queryKeys.allocations.list(workspaceId) : ['allocations', 'disabled'],
...
function invalidateWorkspaceAllocationQueries(queryClient: ReturnType<typeof useQueryClient>, workspaceId: string | undefined) {
  if (!workspaceId) return;
  queryClient.invalidateQueries({ queryKey: queryKeys.allocations.byWorkspace(workspaceId) });
}
...
export function useAddAllocationMutation(workspaceId: string | undefined) {
  ...
  return useMutation({
    ...
    onSuccess: () => invalidateWorkspaceAllocationQueries(queryClient, workspaceId),
  });
}
