...
export default function DiscoverPage() {
  ...
  const removeAllocationMutation = useRemoveAllocationMutation(
    currentWorkspace?.id,
  );
  const promoteAllocationMutation = usePromoteAllocationMutation(
    currentWorkspace?.id,
  );
  const demoteAllocationMutation = useDemoteAllocationMutation(
    currentWorkspace?.id,
  );
  ...
  const [selectedWallet, setSelectedWallet] = useState<DisplayWallet | null>(
    null,
  );
  ...
  const isWalletTracked = (address: string): boolean => {
    ...
    return allocations.some(
      (a) => a.wallet_address.toLowerCase() === lowerAddress,
    );
  };
  ...
  const filteredWallets = useMemo(() => {
    ...
    let result = apiWallets.map((w, i) => ({
      ...
      tracked: allocations.some(
        (a) => a.wallet_address.toLowerCase() === w.address.toLowerCase(),
      ),
    }));
    ...
    if (hideBots) {
      ...
      result = result.filter(
        (wallet) => !(wallet.trades > 1000 && wallet.roi30d < 2),
      );
    }
    ...
    try {
      if (isTracked) {
        ...
        toast.info(
          ...
          `${shortenAddress(address)} is no longer being monitored`,
        );
      } else {
        ...
        toast.success(
          "Added to Watching",
          `${shortenAddress(address)} is now being monitored`,
        );
      }
    } catch {
        ...
        <Card className="bg-primary/5 border-primary/20">
          <CardContent className="p-4">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <span className="text-sm text-muted-foreground">
                {timePeriodLabel.toLowerCase()} ago...
              </span>
            </div>
          </CardContent>
        </Card>
