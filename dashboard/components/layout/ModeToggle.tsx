...
export function ModeToggle() {
  ...
  const demoIsLoading = useDemoPortfolioStore((state) => state.isLoading);
    ...
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        className={cn(
          ...
          "border",
          isTransitioning ? "cursor-wait opacity-60" : "cursor-pointer",
          isDemo
      ...
      {isDemo && (
        <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted text-sm">
          ...
          <span className="font-medium">
            {demoIsLoading ? "—" : formatBalance(demoBalance)}
          </span>
        </div>
      )}
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted text-sm">
            ...
            {walletBalance != null && (
              <>
                ...
                <span className="font-medium">
                  {formatBalance(walletBalance.usdc_balance)}
                </span>
              </>
            )}
          </div>
